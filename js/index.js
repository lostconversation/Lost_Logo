...
function init() {
  ...
  loader.load('font/Rajdhani_Regular.json', function(font) {
    ...
    material3 = new THREE.MeshBasicMaterial({color: 0xffffff, transparent: true, opacity: 0, side: THREE.DoubleSide});
...
function createScene(argument) {
  ...
  galleryData[currentImage].forEach((el, index) => {
    nParticles += .0001;
    // material.size +=.0001,
    geometry.vertices.push(new THREE.Vector3(el.x - Math.random() * 3, el.y - Math.random() * 3, Math.random() * (options.resolution / nParticles)));
  });
  ...
  t0 = new TimelineLite()
  t0.from(pointCloud.position, 5, {
    ease: CustomEase.create("custom", coolEase),
    y: 0,
    z: 1006
  }, 0).from(pointCloud.scale, 5, {
    ease: CustomEase.create("custom", coolEase),
    y: .00004,
    x: .00004
  }, 0);

  lostTxt.traverse(function(object) {
  ...
  t1 = new TimelineLite()
  t1.to(name1.position, 4, {
    z: 0,
    ease: CustomEase.create("custom", coolEase)
  }, 2).to(name2.position, 4, {
    z: 0,
    ease: CustomEase.create("custom", coolEase)
  }, 3).to(name3.position, 4, {
    z: 0,
    ease: CustomEase.create("custom", coolEase)
  }, 4.2).to(name4.position, 4, {
    z: 0,
    ease: CustomEase.create("custom", coolEase)
  }, 4).to(dome.scale, 5, {
    x: .5,
  ...
  }, 3).to(camera.position, 7, {
    z: 200,
    ease: CustomEase.create("custom", coolEase)
  }, 2).to(pointCloud.position, 2, {
    z: -200,
    ease: CustomEase.create("custom", coolEase)
  }, 5)
...
function clone1() {
  ...
  material = new THREE.MeshBasicMaterial({color: 0x2f2f2f, wireframe: true, transparent: true});
  ...
  tlHello.fromTo(newCloud.position, 3, {
...
function shoot() {
  ...
  let tlHello = new TimelineMax();
  tlHello.fromTo(shot.position, 4, {
    ease: Power2.easeInOut,
    x: shotX,
    y: shotY,
    z: 1000
